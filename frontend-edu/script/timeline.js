const Timeline = (() => {
  ...
  function push(emotion, confidence) {
    ...
    if (history.length > CONFIG.TIMELINE_MAX_POINTS) 
      history.shift();
  ...
  function render() {
    ...
    if (summaryEl) {
      summaryEl.innerHTML = top.map(([e, n]) =>
        `<span class="tl-chip" style="background:${CONFIG.EMOTION_COLORS[e]}22;border-color:${CONFIG.EMOTION_COLORS[e]}44;color:${CONFIG.EMOTION_COLORS[e]}">
          ${e} <b>${n}</b>
        </span>`
      ).join("");
    }
