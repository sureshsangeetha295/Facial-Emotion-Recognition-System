...
executor = ThreadPoolExecutor(max_workers=2)   # Increased to 2 for better live performance
...
def _preload():
    try:
        ...
        testing.get_model()
        print("[EngageAI] Models preloaded successfully")
    except Exception as e:
        print(f"[EngageAI] Preload error: {e}")
...
app = FastAPI(title="EngageAI - Student Engagement Monitor", lifespan=lifespan)
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        print(f"[EngageAI] Predict error: {exc}")
...
FRONTEND_DIR = ""
for name in ("frontend", "Frontend", "front-end", "Front-End"):
...
if not FRONTEND_DIR:
    raise RuntimeError(f"Frontend folder not found under: {PROJECT_ROOT}")

print(f"[EngageAI] Frontend served from: {FRONTEND_DIR}")
...
if __name__ == "__main__":
    print("Starting EngageAI - Student Engagement Monitor")
